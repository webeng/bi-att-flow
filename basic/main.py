...
def _train(config):
    ...
    for batches in tqdm(train_data.get_multi_batches(config.batch_size, config.num_gpus,
        ...
        # feed_dict = {}
        # for batch, model in zip(batches, models):
        #     _, ds = batch
        #     feed_dict.update(model.get_feed_dict(ds, True))

        # pprint(model.tensor_dict)

        # global_step, S, vals = sess.run(
        #     [model.global_step, model.u_logits, list(model.tensor_dict.values())],
        #     feed_dict=feed_dict
        # )
        # print(pi)
        # global_step, pi, vals = sess.run(
        #     [model.global_step, model.pi, list(model.tensor_dict.values())],
        #     feed_dict=feed_dict
        # )
        # print(pi.shape)
